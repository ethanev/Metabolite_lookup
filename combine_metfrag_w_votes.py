...
def common_molecules(metfrag_data, voted_data):
	...
	try:
		...
		# print('i think this was a assumed H+ or H-, passing for now')
	...
	# print(voted_metabs_names)
	###################COME BACK HERE AND REMOVE STILL WITH LOW FRCATION OF NoExplPeaks to NumberPeaksUsed
	# print(list(metfrag_data)) 
	...
	for mfrag_name, inchi_met,score, num_peaks in zip(metfrag_data['Name'],metfrag_data['InChI'],metfrag_data['Score'],metfrag_data['NoExplPeaks']):
		...
		for j,inchi_vote in enumerate(multi_voted_inchi):
			...
			if inchi_met_mod in inchi_vote:
				# name = voted_data[1][j][1]
				# common_inchi_name.append((name, inchi_met,score))
				# best_guess.append((str(name).lower(), score))
	...
	metfrag_other_guess = [guess for guess in metfrag_other_guess if guess not in best_guess]
			# if '/'.join(i_1[1].split('/')[:4]) == '/'.join(i_2[1].split('/')[:4]) and i_1[0] != i_2[0]:
				# if (i_2[0],i_2[2],i_1[0], i_1[2]) not in same_inchi and (i_1[0],i_1[2],i_2[0], i_2[2]) not in same_inchi:
				# 	print('not in it!', i_1[2], i_2[2])
				# 	if i_1[2] > i_2[2]:
				# 		same_inchi.append((i_1[0],i_1[2],i_2[0],i_2[2]))
				# 	else:
				# 		same_inchi.append((i_2[0],i_2[2],i_1[0],i_1[2]))
		# for i_2 in common_inchi_name:
		# 	if i_1[1] == i_2[1] and i_1[0] != i_2[0]:
		# 		if (i_1[0],i_2[0]) not in same_inchi and (i_2[0],i_1[0]) not in same_inchi:
		# 			same_inchi.append((i_1[0],i_2[0]))
	# print(same_inchi)
	# if len(same_inchi)>0:
		# reduced_names = reduce_redundancies(same_inchi)
		# print(best_guess, reduced_names)
		# best_guess = [(ele[0].lower(),ele[1]) for ele in best_guess if ele[0] in reduced_names]
		# print(best_guess)
	# else:
		# best_guess = [(ele[0].lower(),ele[1]) for ele in best_guess]

		#sorted(list(set(best_guess)),key=lambda x: x[1], reverse=True)
...
def best_metab_guess(df, mode):
	...
	for i, row in df.iterrows():
		...
		if mode == 'negative':
			...
			# for ele in sorted(guesses.keys()):
			# 	if len(guesses[str(ele)]) != 0:
			# 		best = sorted(guesses[str(ele)], reverse=True, key=lambda x:x[1])
			# 		if len(best) > 1:
			# 			top_guess = [ele for ele in best if ele[1] == best[0][1]]
			# 		else:
			# 			top_guess = best
			...
			for ind in inds:
				try:
					...
					if not seen_nan:
						...
						pass
					# voted_data.loc[row_index][ind]
			votes = add_dict[single_guess['adduct']]
			# for adduct in add_dict:
			# 	if vote in add_dict[adduct]:
			# 		votes = add_dict[adduct]
			# 	else:
			# 		votes = add_dict[single_guess['adduct']]
			for v in votes:
...
def main():
	...
	#### run the following instead since it comes with smiles!
	## for the first time its run: 
	#python combine_metfrag_w_votes.py -f './all_voted_metabs_longitudinal_MIT_50000noise_metabolites_ms2.csv' -c 2 -r 3 -m './ms-ms2/msms_out/' -v 'true' -n 'True' -o './rbc_pos.csv'
